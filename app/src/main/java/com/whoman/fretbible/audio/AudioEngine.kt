...
try{local.startRecording();val buffer=ShortArray(8192);while(running.get()){val count=local.read(buffer,0,buffer.size,AudioRecord.READ_BLOCKING);if(!running.get())break;if(count==buffer.size)_detected.value=detector.detect(FloatArray(count){i->buffer[i]/32768f})}}catch(_:Throwable){}finally{try{local.stop()}catch(_:Throwable){}}}}
fun stop(){running.set(false);val local=recorder;recorder=null;try{local?.stop()}catch(_:Throwable){};try{local?.release()}catch(_:Throwable){};_detected.value=null}}
